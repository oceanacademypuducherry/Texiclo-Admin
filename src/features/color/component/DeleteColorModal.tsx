...
import { deleteColor, setColorDelete } from "../redux";

export const DeleteColormodal = () => {
  const dispatch = useDispatch();
  const { id } = useSelector((state: RootState) => state.color);

  const handleClose = () => {
    dispatch(setColorDelete(false));
  };

  const handleDelete = () => {
    if (id) {
      dispatch(deleteColor({ id, colorName: "", colorValue: "" }));
      dispatch(setColorDelete(false));
    }
  };
  return (
